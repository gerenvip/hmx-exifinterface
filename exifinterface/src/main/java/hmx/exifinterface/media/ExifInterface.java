...
public class ExifInterface {
     ...
     * tream data. Audio files shall be recorded in conformance to the Exif audio format.</p>
     *
     * <ul>
     *     <li>Tag = 40964</li>
     *     <li>Type = String</li>
    ...
    private static class ExifAttribute {
        ...
        Object getValue(ByteOrder byteOrder) {
            ...
            try {
                ...
                log(TAG, "IOException occurred during reading a value :" + e.getMessage());
                return null;
            } finally {
                if (inputStream != null) {
                    try {
                        ...
                        log(TAG, "IOException occurred while closing InputStream", e);
                    }
                }
            }
        }
...
//            IFD_TYPE_ORF_CAMERA_SETTINGS, IFD_TYPE_ORF_IMAGE_PROCESSING, IFD_TYPE_PEF})
     ...
     */
    public ExifInterface(@NotNull File file) throws IOException {
        if (file == null) {
     ...
     *                              {@link FileInputStream#getFD()}.
     */
    public ExifInterface(@NotNull String filename) throws IOException {
        if (filename == null) {
     ...
     */
    public ExifInterface(@NotNull FileDescriptor fileDescriptor) throws IOException {
...
//        }
     ...
     */
    public ExifInterface(@NotNull InputStream inputStream) throws IOException {
     ...
     *                              {@link FileInputStream#getFD()}.
     */
    public ExifInterface(@NotNull InputStream inputStream, @ExifStreamType int streamType)
            throws IOException {
        this(inputStream, (streamType == STREAM_TYPE_EXIF_DATA_ONLY) ? true : false);
    }

    private ExifInterface(@NotNull InputStream inputStream, boolean shouldBeExifDataOnly)
            throws IOException {
        if (inputStream == null) {
        ...
        if (shouldBeExifDataOnly) {
            ...
            if (!isExifDataOnly((BufferedInputStream) inputStream)) {
                log(TAG, "Given data does not follow the structure of an Exif-only data.");
                return;
            }
...
//                mAssetInputStream = null;
     ...
     */
    public static boolean isSupportedMimeType(@NotNull String mimeType) {
        if (mimeType == null) {
    ...
    @Nullable
    private ExifAttribute getExifAttribute(@NotNull String tag) {
        if (tag == null) {
        ...
        if (TAG_ISO_SPEED_RATINGS.equals(tag)) {
            if (BuildConfig.DEBUG) {
                log(TAG, "getExifAttribute: Replacing TAG_ISO_SPEED_RATINGS with "
     ...
     */
    @Nullable
    public String getAttribute(@NotNull String tag) {
        if (tag == null) {
            throw new NullPointerException("tag shouldn't be null");
        }
        ...
        if (attribute != null) {
            ...
            if (tag.equals(TAG_GPS_TIMESTAMP)) {
                ...
                if (attribute.format != IFD_FORMAT_URATIONAL
                    ...
                    log(TAG, "GPS Timestamp format is not rational. format=" + attribute.format);
                    return null;
                }
                Rational[] array = (Rational[]) attribute.getValue(mExifByteOrder);
                if (array == null || array.length != 3) {
                    log(TAG, "Invalid GPS Timestamp array. array=" + Arrays.toString(array));
                    return null;
                }
     ...
     */
    public int getAttributeInt(@NotNull String tag, int defaultValue) {
        if (tag == null) {
            throw new NullPointerException("tag shouldn't be null");
        }
     ...
     * @param defaultValue the value to return if the tag is not available.
     */
    public double getAttributeDouble(@NotNull String tag, double defaultValue) {
        if (tag == null) {
            throw new NullPointerException("tag shouldn't be null");
        }
        ExifAttribute exifAttribute = getExifAttribute(tag);
    ...
    public void setAttribute(@NotNull String tag, @Nullable String value) {
        ...
        if (TAG_ISO_SPEED_RATINGS.equals(tag)) {
            if (BuildConfig.DEBUG) {
                log(TAG, "setAttribute: Replacing TAG_ISO_SPEED_RATINGS with "
                        + "TAG_PHOTOGRAPHIC_SENSITIVITY.");
            }
            tag = TAG_PHOTOGRAPHIC_SENSITIVITY;
        }
        ...
        if (value != null && sTagSetForCompatibility.contains(tag)) {
            if (tag.equals(TAG_GPS_TIMESTAMP)) {
                ...
                if (!m.find()) {
                    log(TAG, "Invalid value for " + tag + " : " + value);
                    return;
                }
        ...
        for (int i = 0; i < EXIF_TAGS.length; ++i) {
            ...
            if (exifTag != null) {
                ...
                int dataFormat;
                if (exifTag.primaryFormat == guess.f || exifTag.primaryFormat == guess.s) {
                    dataFormat = exifTag.primaryFormat;
                } else if (exifTag.secondaryFormat != -1 && (exifTag.secondaryFormat == guess.f
                        || exifTag.secondaryFormat == guess.s)) {
                    ...
                    if (BuildConfig.DEBUG) {
                        log(TAG, "Given tag (" + tag
                                ...
                                + IFD_FORMAT_NAMES[guess.f] + (guess.s == -1 ? "" : ", "
                                + IFD_FORMAT_NAMES[guess.s]) + ")");
                    }
                    continue;
                }
                switch (dataFormat) {
                    ...
                    default:
                        if (BuildConfig.DEBUG) {
                            log(TAG, "Data format isn't one of expected formats: " + dataFormat);
                        }
                        continue;
                }
            }
     ...
     */
    private void loadAttributes(@NotNull InputStream in) {
        if (in == null) {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, "Invalid image: ExifInterface got an unsupported image format file"
...
//            } catch (Exception e) {
//                if (BuildConfig.DEBUG) {
//                    log(TAG, "The file descriptor for the given input is not seekable");
//                }
//                return false;
//            }
//        }
     ...
     * @param tag the name of the tag.
     */
    public boolean hasAttribute(@NotNull String tag) {
        ...
        try {
            /*if (mAssetInputStream != null) {
                ...
                if (in.markSupported()) {
                    ...
                    log(TAG, "Cannot read thumbnail from inputstream without mark/reset support");
                    return null;
                }
            } else*/
            if (mFilename != null) {
            ...
            log(TAG, "Encountered exception while getting thumbnail", e);
        } finally {
            closeQuietly(in);
     ...
     */
    @Nullable
    public PixelMap getThumbnailBitmap() {
        if (!mHasThumbnail) {
            return null;
        ...
        if (mThumbnailCompression == DATA_JPEG || mThumbnailCompression == DATA_JPEG_COMPRESSED) {
            return BitmapUtils.decodeBitmap(mThumbnailBytes);
//            return BitmapFactory.decodeByteArray(mThumbnailBytes, 0, mThumbnailLength);
            ...
            if (imageLengthAttribute != null && imageWidthAttribute != null) {
                ...
                options.size = new Size(imageWidth, imageLength);
                return PixelMap.create(rgbValues, options);
...
//                        rgbValues, imageWidth, imageLength, Bitmap.Config.ARGB_8888);
     ...
     */
    @Nullable
    public long[] getAttributeRange(@NotNull String tag) {
        if (tag == null) {
            throw new NullPointerException("tag shouldn't be null");
        }
        if (mModified) {
     ...
     */
    @Nullable
    public byte[] getAttributeBytes(@NotNull String tag) {
        if (tag == null) {
            throw new NullPointerException("tag shouldn't be null");
        }
        final ExifAttribute attribute = getExifAttribute(tag);
    ...
    public double[] getLatLong() {
        ...
        if (latValue != null && latRef != null && lngValue != null && lngRef != null) {
            try {
                ...
                log(TAG, "Latitude/longitude values are not parsable. "
...
//        setAttribute(ExifInterface.TAG_GPS_PROCESSING_METHOD, location.getProvider());
        ...
        String[] dateTime = sFormatter.format(new Date(location.getTimeStamp())).split("\\s+", -1);
...
//        mAssetInputStream = null;
    ...
    private boolean isHeifFormat(byte[] signatureCheckBytes) throws IOException {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, "Exception parsing HEIF file type box.", e);
            }
        } finally {
    ...
    private void getJpegAttributes(ByteOrderedDataInputStream in, int jpegOffset, int imageType)
        ...
        if (BuildConfig.DEBUG) {
            log(TAG, "getJpegAttributes starting with: " + in);
        }
        ...
        while (true) {
            ...
            marker = in.readByte();
            if (BuildConfig.DEBUG) {
                log(TAG, "Found JPEG segment indicator: " + Integer.toHexString(marker & 0xff));
            }
            ++bytesRead;
            ...
            bytesRead += 2;
            if (BuildConfig.DEBUG) {
                log(TAG, "JPEG segment: " + Integer.toHexString(marker & 0xff) + " (length: "
                        + (length + 2) + ")");
            }
            if (length < 0) {
    ...
    private void getRafAttributes(ByteOrderedDataInputStream in) throws IOException {
        ...
        if (BuildConfig.DEBUG) {
            log(TAG, "numberOfDirectoryEntry: " + numberOfDirectoryEntry);
        }
        ...
        for (int i = 0; i < numberOfDirectoryEntry; ++i) {
            ...
            if (tagNumber == TAG_RAF_IMAGE_SIZE.number) {
                ...
                if (BuildConfig.DEBUG) {
                    log(TAG, "Updated to length: " + imageLength + ", width: " + imageWidth);
                }
                return;
            }
    ...
    private void getHeifAttributes(final ByteOrderedDataInputStream in) throws IOException {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, "Heif meta: " + width + "x" + height + ", rotation " + rotation);
            }
        } finally {
            retriever.release();
        }*/
        ...
        if (makerNoteAttribute != null) {
            ...
            if (aspectFrameAttribute != null) {
                ...
                if (aspectFrameValues == null || aspectFrameValues.length != 4) {
                    log(TAG, "Invalid aspect frame values. frame="
    ...
    private void getPngAttributes(ByteOrderedDataInputStream in) throws IOException {
        if (BuildConfig.DEBUG) {
            log(TAG, "getPngAttributes starting with: " + in);
        }
    ...
    private void getWebpAttributes(ByteOrderedDataInputStream in) throws IOException {
        if (BuildConfig.DEBUG) {
            log(TAG, "getWebpAttributes starting with: " + in);
        }
    ...
    private void saveJpegAttributes(InputStream inputStream, OutputStream outputStream)
        ...
        // See JPEG File Interchange Format Specification, "JFIF Specification"
        if (BuildConfig.DEBUG) {
            log(TAG, "saveJpegAttributes starting with (inputStream: " + inputStream
    ...
    private void savePngAttributes(InputStream inputStream, OutputStream outputStream)
            throws IOException {
        if (BuildConfig.DEBUG) {
            log(TAG, "savePngAttributes starting with (inputStream: " + inputStream
                    + ", outputStream: " + outputStream + ")");
        }
        DataInputStream dataInputStream = new DataInputStream(inputStream);
        ByteOrderedDataOutputStream dataOutputStream =
    ...
    private void saveWebpAttributes(InputStream inputStream, OutputStream outputStream)
            throws IOException {
        if (BuildConfig.DEBUG) {
            log(TAG, "saveWebpAttributes starting with (inputStream: " + inputStream
                    + ", outputStream: " + outputStream + ")");
        }
    ...
    private ByteOrder readByteOrder(ByteOrderedDataInputStream dataInputStream)
        ...
        switch (byteOrder) {
            case BYTE_ALIGN_II:
                if (BuildConfig.DEBUG) {
                    log(TAG, "readExifSegment: Byte Align II");
                }
                return ByteOrder.LITTLE_ENDIAN;
            case BYTE_ALIGN_MM:
                if (BuildConfig.DEBUG) {
                    log(TAG, "readExifSegment: Byte Align MM");
                }
    ...
    private void readImageFileDirectory(ByteOrderedDataInputStream dataInputStream,
        ...
        for (short i = 0; i < numberOfDirectoryEntry; ++i) {
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, String.format("ifdType: %d, tagNumber: %d, tagName: %s, dataFormat: %d, "
            ...
            if (tag == null) {
                if (BuildConfig.DEBUG) {
                    log(TAG, "Skip the tag entry since tag number is not defined: " + tagNumber);
                }
            } else if (dataFormat <= 0 || dataFormat >= IFD_FORMAT_BYTES_PER_FORMAT.length) {
                if (BuildConfig.DEBUG) {
                    log(TAG, "Skip the tag entry since data format is invalid: " + dataFormat);
                }
            } else if (!tag.isFormatCompatible(dataFormat)) {
                if (BuildConfig.DEBUG) {
                    log(TAG, "Skip the tag entry since data format ("
                ...
                if (byteCount < 0 || byteCount > Integer.MAX_VALUE) {
                    if (BuildConfig.DEBUG) {
                        log(TAG, "Skip the tag entry since the number of components is invalid: "
                                + numberOfComponents);
                    }
                } else {
                    valid = true;
            ...
            if (byteCount > 4) {
                ...
                if (BuildConfig.DEBUG) {
                    log(TAG, "seek to data offset: " + offset);
                }
                ...
                if (offset + byteCount <= dataInputStream.mLength) {
                    ...
                    if (BuildConfig.DEBUG) {
                        log(TAG, "Skip the tag entry since data offset is invalid: " + offset);
                    }
                    dataInputStream.seek(nextEntryOffset);
                    continue;
                }
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, "nextIfdType: " + nextIfdType + " byteCount: " + byteCount);
            }
            ...
            if (nextIfdType != null) {
                ...
                if (BuildConfig.DEBUG) {
                    log(TAG, String.format("Offset: %d, tagName: %s", offset, tag.name));
                }
                ...
                if (offset > 0L && offset < dataInputStream.mLength) {
                    if (!mAttributesOffsets.contains((int) offset)) {
                        ...
                        if (BuildConfig.DEBUG) {
                            log(TAG, "Skip jump into the IFD since it has already been read: "
                                    + "IfdType " + nextIfdType + " (at " + offset + ")");
                        }
                    }
                } else {
                    if (BuildConfig.DEBUG) {
                        log(TAG, "Skip jump into the IFD since its offset is invalid: " + offset);
                    }
                }
        ...
        if (dataInputStream.peek() + 4 <= dataInputStream.mLength) {
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, String.format("nextIfdOffset: %d", nextIfdOffset));
            }
            // Check if the next IFD offset
            // 1. Exists within the boundaries of the input stream
            // 2. Does not point to a previously read IFD.
            if (nextIfdOffset > 0L && nextIfdOffset < dataInputStream.mLength) {
                if (!mAttributesOffsets.contains(nextIfdOffset)) {
                    ...
                    if (BuildConfig.DEBUG) {
                        log(TAG, "Stop reading file since re-reading an IFD may cause an "
                                + "infinite loop: " + nextIfdOffset);
                    }
                }
            } else {
                if (BuildConfig.DEBUG) {
                    log(TAG, "Stop reading file since a wrong offset may cause an infinite loop: "
                            + nextIfdOffset);
                }
            }
        }
    ...
    private void handleThumbnailFromJfif(ByteOrderedDataInputStream in, HashMap thumbnailData)
        ...
        if (jpegInterchangeFormatAttribute != null
            ...
            if (thumbnailOffset > 0 && thumbnailLength > 0) {
                ...
                if (mFilename == null && mSeekableFileDescriptor == null) {
                    // TODO: Need to handle potential OutOfMemoryError
            ...
            if (BuildConfig.DEBUG) {
                log(TAG, "Setting thumbnail attributes with offset: " + thumbnailOffset
    ...
    private void handleThumbnailFromStrips(ByteOrderedDataInputStream in, HashMap thumbnailData)
        ...
        if (stripOffsetsAttribute != null && stripByteCountsAttribute != null) {
            ...
            if (stripOffsets == null || stripOffsets.length == 0) {
                log(TAG, "stripOffsets should not be null or have zero length.");
                return;
            }
            if (stripByteCounts == null || stripByteCounts.length == 0) {
                log(TAG, "stripByteCounts should not be null or have zero length.");
                return;
            }
            if (stripOffsets.length != stripByteCounts.length) {
                log(TAG, "stripOffsets and stripByteCounts should have same length.");
                return;
            }
            ...
            for (int i = 0; i < stripOffsets.length; i++) {
                ...
                if (skipBytes < 0) {
                    log(TAG, "Invalid strip offset value");
                }
                in.seek(skipBytes);
                bytesRead += skipBytes;
    ...
    private boolean isSupportedDataType(HashMap thumbnailData) throws IOException {
        ...
        if (BuildConfig.DEBUG) {
            log(TAG, "Unsupported data type value");
        }
        return false;
    }
    ...
    private void validateImages() throws IOException {
        ...
        if (!isThumbnail(mAttributes[IFD_TYPE_THUMBNAIL])) {
            log(TAG, "No image meets the size requirements of a thumbnail image.");
        }
    }
        ...
        if (defaultCropSizeAttribute != null) {
            ...
            if (defaultCropSizeAttribute.format == IFD_FORMAT_URATIONAL) {
                ...
                if (defaultCropSizeValue == null || defaultCropSizeValue.length != 2) {
                    log(TAG, "Invalid crop size values. cropSize="
    ...
    private int writeExifSegment(ByteOrderedDataOutputStream dataOutputStream) throws IOException {
        ...
        if (BuildConfig.DEBUG) {
            for (int i = 0; i < EXIF_TAGS.length; ++i) {
                log(TAG, String.format("index: %d, offsets: %d, tag count: %d, data sizes: %d, "
    ...
    private static Pair<Integer, Integer> guessDataFormat(String entryValue) {
        ...
        if (entryValue.contains(",")) {
            ...
            if (dataFormat.f == IFD_FORMAT_STRING) {
                return dataFormat;
            }
            for (int i = 1; i < entryValues.length; ++i) {
                final Pair<Integer, Integer> guessDataFormat = guessDataFormat(entryValues[i]);
                int first = -1, second = -1;
                if (guessDataFormat.f.equals(dataFormat.f)
                        || guessDataFormat.s.equals(dataFormat.f)) {
                    first = dataFormat.f;
                }
                if (dataFormat.s != -1 && (guessDataFormat.f.equals(dataFormat.s)
                        || guessDataFormat.s.equals(dataFormat.s))) {
                    second = dataFormat.s;
                }
                if (first == -1 && second == -1) {
    ...
    private void swapBasedOnImageSize(@IfdType int firstIfdType, @IfdType int secondIfdType)
        ...
        if (mAttributes[firstIfdType].isEmpty() || mAttributes[secondIfdType].isEmpty()) {
            if (BuildConfig.DEBUG) {
                log(TAG, "Cannot perform swap since only one image data exists");
            }
            return;
        }
        ...
        if (firstImageLengthAttribute == null || firstImageWidthAttribute == null) {
            if (BuildConfig.DEBUG) {
                log(TAG, "First image does not contain valid size information");
            }
        } else if (secondImageLengthAttribute == null || secondImageWidthAttribute == null) {
            if (BuildConfig.DEBUG) {
                log(TAG, "Second image does not contain valid size information");
            }
        } else {
...
//            } catch (Exception ex) {
//                LogUtil.e(TAG, "Error closing fd.");
//            }
//        } else {
//            LogUtil.e(TAG, "closeFileDescriptor is called in API < 21, which must be wrong.");
//        }
    }
    ...
    private static void log(String tag, String msg, Throwable throwable) {
        HiLog.debug(LABEL_LOG, LOG_FORMAT, tag, msg + "; e:" + throwable.getMessage());
    }
